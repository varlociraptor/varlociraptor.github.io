...
In case the exact alteration fraction (variant allele frequency/allelic fraction or methylation rate) is less important for you (e.g. with large cohorts), you can add the argument ``--pairhmm-mode fast``.
...
In rare cases this can lead to wrong probabilities for single reads though, such that alteration fractions can become slightly incorrect.
...
In the following example, instead of defining uniform alteration fraction universes as above, we define the inheritance relationship between samples and the properties of the underlying species.
